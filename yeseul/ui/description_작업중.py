...
그 중 **지역별 판매 실적 데이터**는 다음과 같이 구성되어 있습니다:

- **기아차**  
  모델명이 **행(row)** 으로, 국가명이 **열(column)** 로 구성되어  
  **국가별 차종 판매 현황**을 직관적으로 비교할 수 있는 형태입니다.

- **현대차**  
  **EU와 미국** 각각에 대해 **별도의 파일**로 제공되었으며,  
  **월별 판매 실적 데이터**가 포함되어 있어  
  **시간 흐름에 따른 지역별 판매 트렌드 분석**에 적합한 구조입니다.
...
### 📈 데이터 전처리 결과""")
    ...
    st.markdown("---")
    st.markdown("""
### 📈 지역별 Prophet 예측 모델 제작과정""")
    df1=pd.read_csv('data/현대_지역별수출실적.csv')
    df2=pd.read_csv('data/현대_지역별수출실적_melt.csv')
    st.markdown("""
1. 원본 데이터를 **Prophet 모델**에 적합한 형태로 변환하기 위해
   `pd.melt()` 함수를 활용하여 **시계열 데이터**로 wide → long 변환하였습니다.
    """)

    st.write('**원본 데이터**')
    st.markdown("""
- 행(row): "국가", "연도" 별로 하나의 레코드
- 열(column): 1월 ~ 12월까지의 월별 수출 실적 수치 포함""")
    st.dataframe(df1.head(),hide_index=True)
    ...
    - `"날짜"` → **`ds`** (Date Stamp, 날짜 정보, Prophet 예측을 위해 정렬처리)
    - `"수출실적"` → **`y`** (예측 대상 값)  
    """, unsafe_allow_html=True)
        
    st.markdown("""
Prophet은 기본적으로 일간(daily), 주간(weekly), 연간(yearly) 계절성을 자동으로 탐지하여 적용합니다.<br>
그러나 분석 데이터는 **월별 수출 실적 데이터**로, 일 단위 seasonality는 존재하지 않습니다.
                """,unsafe_allow_html=True)
    st.code("model = Prophet(daily_seasonality=False)")
    st.markdown("""
- 존재하지 않는 일간 패턴을 학습하려는 것을 방지  
- **과적합(Overfitting)** 및 불필요한 계산 리소스를 줄이기 위해  
- `daily_seasonality=False` 설정으로 **일간 계절성 비활성화**
                """,unsafe_allow_html=True)

    df1=pd.read_csv('data/prophet_results.csv')
    col1, col2 = st.columns(2)
    with col1:
        st.write('Mape 결과')
    ...
    with col2:
        st.write('예측 결과 그래프')
        st.image('image/북미-멕시코_수출실적.png')

    st.success("""
    🌟 국가별 Prophet 예측 시스템  
    Prophet 모델을 기반으로, 국가별 월별 수출 실적 데이터를 학습하여  
    향후 18개월의 수출 실적을 예측합니다.

    - Prophet의 자동 추세 및 계절성 탐지 기능 활용  
    - `daily_seasonality=False` 설정으로 월 단위 시계열 최적화  
    - 훈련된 모델은 국가별 `.pkl` 파일로 저장  
    - 예측값 + 신뢰구간 시각화로 인사이트 제공
    """)
    ...
    - 수치 예측 외에 **최근 정성적 이슈와 동향**을 함께 파악 가능
    ...
    > 👉 판매량 예측 그래프, 수치 지표 데이터 + 뉴스 기사를 통한 시장 동향 + AI 해석을 종합적으로 제공하여  
    ...
    st.markdown("""
    프롬프트 엔지니어링을 토대로, 예측 지표와 뉴스 키워드를 결합한  
    **AI 기반 시장 보고서**가 자동 생성됩니다.  
    이 보고서는 수출 트렌드, 이슈 요약, 전략 제언까지 포함하여  
    의사결정자에게 **실질적인 인사이트**를 제공합니다.
    """)
